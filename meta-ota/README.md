...
│       ├── swupdate_%.bbappend
│       └── files/
│           ├── defconfig          # build-time: downloader, libubootenv, hw-compat
│           ├── swupdate.cfg.in    # runtime: download retries/timeout, reboot-required
│           └── 09-swupdate-args   # script downloader
│
└── wic/
...
- **`recipes-support/`** — Cấu hình SWUpdate: [`defconfig`](recipes-support/swupdate/files/defconfig) (downloader curl, libubootenv, hw-compat) + [`signed-images.cfg`](recipes-support/swupdate/files/signed-images.cfg) (bật verify chữ ký CMS khi `secure-boot`, xem [changes/0016](../docs/changes/0016-ota-swu-signing.md)), [`swupdate.cfg.in`](recipes-support/swupdate/files/swupdate.cfg.in) (download retries/timeout, **`reboot-required = false`** — Qt app điều khiển reboot, xem [changes/0005](../docs/changes/0005-ota-pull-model-host-server.md)), [`09-swupdate-args`](recipes-support/swupdate/files/09-swupdate-args) (script downloader: nhận URL, chọn install set theo slot đang chạy, gọi `swupdate -d`). Webserver daemon thường trực **đã tắt** (`SYSTEMD_AUTO_ENABLE = disable`) — dùng downloader one-shot, xem [changes/0012](../docs/changes/0012-ota-swupdate-downloader.md).
...
> **Pull-model** (từ [changes/0005](../docs/changes/0005-ota-pull-model-host-server.md), cập nhật [changes/0012](../docs/changes/0012-ota-swupdate-downloader.md)): Qt app trên BBB phát hiện version mới rồi **đưa URL `.swu` cho SWUpdate downloader** để SWUpdate tự tải + flash — thay cho việc admin upload thủ công vào web UI. Phần bootloader/A-B dưới đây **không đổi**.

1. Qt app phát hiện version mới (MQTT auto / HTTP manual), rồi gọi `09-swupdate-args <url>` → spawn **`swupdate -d -u <url>` (one-shot)**; SWUpdate tự `curl` tải `.swu` từ host. (Trước đây: app tải về `/tmp` rồi POST lên webserver `127.0.0.1:8080`.)
...
| Bật/sửa ký `.swu` (CMS) | [`update-image.bb`](recipes-extended/images/update-image.bb) (ký) + [`signed-images.cfg`](recipes-support/swupdate/files/signed-images.cfg) + [`swupdate_%.bbappend`](recipes-support/swupdate/swupdate_%.bbappend) (verify) |
| Bật encryption | [`defconfig`](recipes-support/swupdate/files/defconfig) |
