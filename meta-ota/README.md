...
- **`recipes-support/`** — Cấu hình SWUpdate: [`defconfig`](recipes-support/swupdate/files/defconfig) (downloader curl, libubootenv, hw-compat; không sign/encrypt), [`swupdate.cfg.in`](recipes-support/swupdate/files/swupdate.cfg.in) (download retries/timeout, **`reboot-required = false`** — Qt app điều khiển reboot, xem [changes/0005](../docs/changes/0005-ota-pull-model-host-server.md)), [`09-swupdate-args`](recipes-support/swupdate/files/09-swupdate-args) (script downloader: nhận URL, chọn install set theo slot đang chạy, gọi `swupdate -d`). Webserver daemon thường trực **đã tắt** (`SYSTEMD_AUTO_ENABLE = disable`) — dùng downloader one-shot, xem [changes/0012](../docs/changes/0012-ota-swupdate-downloader.md).
